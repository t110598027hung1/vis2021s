 ...
 * @param {string[]} csvPaths - 一個包含實驗室名單的csv路徑陣列
 * @param {parseTopojsonCallback} callback - 處理 topojson 的callback
 * @returns 
 */
function getMap(topojsonPath, citiesInfoPath, csvPaths, callback){

    /**
     * mycsv接收一個包含csv路徑的陣列，在接受完所有csv之後，會將資料儲存在一個陣列裡
     * 該陣列的格式為: {
     *  filename: 代表該csv的檔名
     *  d: 該csv轉換成為object的資料
     * }
    */
    var mycsv = function(arr, cb){
        var result = [];
        var paths = arr.slice(0, arr.length);
    ...
    return d3.json(citiesInfoPath, function(citiesInfo){
        ...
        return d3.json(topojsonPath, function(topojsonFile){
            ...
            mycsv(csvPaths, function(csv){

                //每兩秒執行一次這個函數
                setInterval(function(){
                    //隨機選取一個csv的資料
                    var choice = csv[Math.floor(Math.random() * csv.length)];
                    
                    //在上方文字區顯示實驗室名稱
                    document.getElementById('lab').innerText = choice.filename.split('.')[0];
                    ...
                    svg.selectAll('circle')
                        ...
                        .append('circle')
                        /**
                         * 依照給定的經緯度帶入myPrjection函式，計算出在svg上相應的位置
                         * 請注意myPrjection必須吃一個陣列，該陣列的第一個元素要放經度，第二個放緯度
                         */
                        .attr('cx', function(d){return myPrjection([d.lon, d.lat])[0]})
                        .attr('cy', function(d){return myPrjection([d.lon, d.lat])[1]})
                        //圓圈的半徑
                        .attr('r', 5)
                        //塗紅色
                        .style('fill', 'red');
                    ...
                    svg.selectAll('text')
                        ...
                        .enter()
                        .append('text')
                        //依照經緯度算出位置，並做小幅度調整，使文字出現紅點右邊
                        .attr('x', function(d){return myPrjection([d.lon, d.lat])[0]+8})
                        .attr('y', function(d){return myPrjection([d.lon, d.lat])[1]+5})
